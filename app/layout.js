      ...
      <body className={`${geistSans.variable} ${geistMono.variable} antialiased bg-white text-slate-900`}>
        ...
        <nav className="w-full bg-white border-b border-gray-100 sticky top-0 z-[100] shadow-sm">
          <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            {/* LOGO */}
            <Link href="/" className="text-xl font-black italic tracking-tighter shrink-0 z-[102]">
              BIHIN <span className="text-blue-600">NISAN</span>
            </Link>

            {/* TOMBOL HAMBURGER - Dipaksa muncul di layar HP & Tablet */}
            <button 
              onClick={() => setIsOpen(!isOpen)}
              className="lg:hidden p-2 text-slate-600 z-[102] bg-gray-50 rounded-lg"
            >
            ...
            {/* MENU DESKTOP - HANYA muncul di layar Lebar (Laptop) */}
            <div className="hidden lg:flex items-center gap-6 font-semibold text-sm">
              ...
              <Link href="/lokasi" className="hover:text-blue-600">Lokasi</Link>
              <Link href="/lacak" className="bg-blue-600 text-white px-4 py-2 rounded-lg">Lacak Pesanan</Link>
            </div>
          </div>

          {/* MENU MOBILE - Overlay Full Screen agar tidak tabrakan */}
          {isOpen && (
            <div className="fixed inset-0 bg-white z-[101] flex flex-col p-8 pt-24 animate-in fade-in slide-in-from-top-5">
              <div className="flex flex-col gap-6 text-2xl font-bold">
                ...
                <Link onClick={() => setIsOpen(false)} href="/lacak" className="bg-blue-600 text-white p-5 rounded-2xl text-center">Lacak Pesanan</Link>
              </div>
            </div>
          )}
        </nav>
